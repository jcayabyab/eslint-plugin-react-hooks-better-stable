...
export default {
  meta: {
    ...
    docs: {
      ...
      recommended: true,
      url: 'https://github.com/jcayabyab/eslint-plugin-react-hooks-better-stable/blob/main/README.md',
    },
    fixable: 'code',
    hasSuggestions: true,
    schema: [
  ...
  create(context) {
    ...
    const knownStableValues =
      context.options && context.options[0] && context.options[0].knownStableValues
        ? new RegExp(context.options[0].knownStableValues)
        : undefined;

    const markStableValuesAsUnnecessary =
      context.options && context.options[0] && context.options[0].markStableValuesAsUnnecessary;
    ...
    function visitCallExpression(node) {
      ...
      switch (callback.type) {
        ...
        case 'Identifier':
          ...
          break; // Unhandled
        case 'CallExpression':
          if (options.knownStableValues && options.knownStableValues.test(callback.callee.name)) {
            return; // Handled
          }
